...
class _QuickStartPageState extends State<QuickStartPage> {
  ...
  void initState() {
    ...
    if(_Videoparameter["description"] == "tuisongAudio"){
      print("进入推送");
    ...
    if(_Videoparameter["description"] == "tuisongAudio" ||  _Videoparameter["description"] == "jieshou") {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        child: Stack(
          children: <Widget>[_viewAudio(), _isVideo? _bottomToolBar():_bottomToolBar2()],
        ),
      ),
    );
  }
