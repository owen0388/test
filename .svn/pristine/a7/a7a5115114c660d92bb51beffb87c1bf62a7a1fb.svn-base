...
import 'QuickStart/quick_start/quick_start_page.dart';
...
class ChatLogic extends GetxController {
  ...
  void onInit() {
    ...
    imLogic.onRecvNewMessage = (Message message) {
      ...
      var info = jsonDecode(message.content.toString());
      print(info["Flag"]);

      if (message.contentType == MessageType.typing && info["Flag"] == "tuisongVideo"){
        print("视频请求");
        AppNavigator.OpenQuickStartVideoPage(info["RoomID"].toString(),info["TuiSong"].toString(),"jieshou");
      }

      if (message.contentType == MessageType.typing && info["Flag"] == "tuisongAudio"){
        print("语音请求");
        AppNavigator.OpenQuickStartPage(info["RoomID"].toString(),info["TuiSong"].toString(),"jieshou");
      }

      if (message.contentType == MessageType.typing && info["Flag"] == "guaduan"){
  ...
  /// 提示对方正在输入
  void sendTypingMsg_forVideo(String paramdate) async {
  ...
  void call(BuildContext context) {
    print("音频视频通话");
    // AppNavigator.OpenQuickStartPage();
              ...
              children: <Widget>[
                TextButton(
                  ...
                  onPressed: () {
                    ...
                    var param = Map();
                    param["RoomID"] =imLogic.userInfo.value.userID.toString();//房间号
                    param["TuiSong"] =imLogic.userInfo.value.userID.toString();//推流
                    param["JieShou"] =uid.toString();//接收流
                    param["Flag"] ="tuisongVideo";//标志

                    sendTypingMsg_forVideo(json.encode(param));
                    AppNavigator.OpenQuickStartVideoPage(imLogic.userInfo.value.userID.toString(),uid.toString(),"tuisongVideo");
                  },
                ),
                TextButton(
                  child: Text("语音通话"),
                  onPressed: () {
                    ...
                    param["TuiSong"] =imLogic.userInfo.value.userID.toString();//推流
                    param["JieShou"] =uid.toString();//接收流
                    param["Flag"] ="tuisongAudio";//标志

                    sendTypingMsg_forVideo(json.encode(param));
                    AppNavigator.OpenQuickStartPage(imLogic.userInfo.value.userID.toString(),uid.toString(),"tuisongAudio");
                  },
                ),
                TextButton(
                  child: Text("取消"),
